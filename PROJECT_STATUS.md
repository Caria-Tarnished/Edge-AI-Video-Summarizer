...
- 按 Architecture_Design 的里程碑推进（建议顺序）：
  ...
  - **v0.0.5 计划（功能增强与体验修复）**：
    - [ ] **ASR 文本前置纠错 (LLM Transcript Polish)**：在 Whisper 转写完毕后，按时间块调用本地大模型对字幕进行错别字/同音字修正（保留时间戳），为下游索引和摘要提供干净的高质量文本。
    - [ ] **语义级关键帧拆分 (Semantic Keyframes)**：探索基于摘要大纲 / 文本视听特征的更智能的抽帧模式（不仅仅是按固定 10s 或纯视觉场景突发变更抽帧）。
    - [ ] **摘要质量跃升**：引入 Map-Reduce 长文本分块摘要范式，优化 Prompt，通过结构化输出（JSON Mode）收敛大模型的发散问题，保证摘要逻辑清晰。
    - [ ] **修复 Bug（状态栏丢失）**：修复前端在切换 Tab/页面（工作区）并切回后，转写、索引、摘要或关键帧的进度条被清空重置为 `-` 的问题。需排查 Zustand 状态管理或 React useEffect 加载时机漏发事件的问题。
    - [ ] **恢复云端 LLM 确认弹窗**：当用户选择 `openai_cloud`（`requires_confirm_send=true`）provider 时，前端在调用摘要/聊天前应弹出确认对话框告知用户数据将发送到外部服务器，经用户确认后再发请求。此逻辑在 v0.0.4 修复本地 LLM 误弹窗时被误删。
    - [ ] **Settings 页云端 LLM 接入完善**：
      - **自定义 API Key 入口**：在设置页新增 API Key 输入框，允许用户填写自己的云端大模型 Key（当前仅支持环境变量 `LLM_CLOUD_API_KEY`）；未配置时给出红色警告提示。
      - **网络代理配置**：新增可选的 HTTP/SOCKS5 代理地址输入框（如 `http://127.0.0.1:7890`），解决中国大陆等地区用户无法直连 OpenAI/Anthropic API 的问题。后端 `urllib` 请求通过 `ProxyHandler` 转发。
      - **托管付费 API（开发者运营）**：提供一种"开箱即用"的付费云端模式——开发者预置几个主流大模型的 API 中转端点，用户无需自行申请 Key，仅需支付略高于 token 成本的费用即可使用。自定义 API Key 入口始终保留，付费模式为可选增值服务。
  - **远期规划（用户体验优化级）**：
    - [ ] **视频源集成下载器 (Video Downloader)**：支持从外网直接拉取视频（如 Bilibili、YouTube）并接入处理管线，解决 B 站本地音视频分离的解码/重新封装问题以及 YouTube 的下载风控问题，彻底打通“输入网址 -> 产出总结”的无缝闭环。
    - [ ] **“一键解析”自动化流水线**：为小白用户提供一个总按钮，一键按顺序触发“转写 -> 索引 -> 大纲与摘要 -> 关键帧”，代替当前繁琐的四个独立按钮人工点击。
    - [ ] **人类可读的时间戳格式化**：将摘要和笔记中的纯秒数时间戳（如 `850s`）转为易读的时分秒格式（如 `14:10`）。
    - [ ] **开箱即用与启动自动化**：应用启动时，自动完成各基础组件（GPU、CUDA 依赖、Whisper 模型）的自检。
    - [ ] **内置 LLM 守护进程**：集成启动策略，运行程序后自动在后台拉起 `llama-server`（无需用户跑到“设置”中手动清除日志并点击启动）。
    - [ ] **精简体验**：逐步收敛并隐藏过于技术化的配置页内容，降低普通用户的理解门槛。
...
### P3-5：数据目录迁移 / 备份 / 恢复（桌面端打包版）验证备忘

> 目标：**打包版应用**默认将后端数据目录迁移到 Electron 的 `app.getPath('userData')` 下；并提供“一键导出 zip / 一键恢复 zip”。

#### 0) 为什么你在前端界面没有看到“迁移入口/迁移页面”？

- **迁移不是一个页面/按钮**。
- 迁移逻辑发生在 **Electron 主进程启动后端之前**，只有在满足条件时，才会在启动时弹一个系统对话框。
- 同时，“导出/恢复 zip”按钮目前也做了 **仅打包版显示**：
  - 你用 `start_dev.cmd` 启动的是开发模式（非打包），因此不会出现迁移弹窗，也不会显示导出/恢复按钮。

#### 1) 前置条件（必须是“打包版应用”）

- 你需要运行打包后的应用（例如 `release/**/win-unpacked/` 下的 exe，或安装包安装后的 exe）。
- 打包版启动后，后端会由 Electron 主进程启动（不会依赖你手动在命令行启动 backend）。

#### 2) 关键路径说明（用于检查是否真的生效）

- **旧默认目录（legacy）**：`%USERPROFILE%\.edge-video-agent`
  - 例：`C:\Users\<你用户名>\.edge-video-agent`
- **打包版新默认目录（packaged default）**：`<Electron userData>\edge-video-agent-data`
  - 具体 `userData` 在不同机器/安装方式可能不同；最稳妥的办法是：
    - 打开应用 `Settings -> Diagnostics -> 刷新`，查看后端返回的 `data_dir` 字段（这就是当前实际使用的数据目录）。

#### 3) 验证“启动时迁移弹窗”出现的复现步骤（建议严格按步骤）

1. 关闭应用（确保没有 `Edge Video Agent` 相关进程占用文件）。
2. 准备旧目录（若不存在就创建），并放入一个明显的“哨兵文件”，用于确认是否被复制：
...
3. 确保新目录是“空的”（关键：**只有新目录为空时才会触发迁移**）：

   - 新目录一般在 `userData\edge-video-agent-data`。
   - 如果你之前运行过打包版，它可能已经有内容；此时迁移弹窗不会出现。
   - 你可以先手动备份并删除新目录（谨慎操作），或换一台干净环境/新 Windows 用户。
4. 启动打包版应用。
5. 预期：会弹出对话框提示检测到旧版数据目录，并给出两个选项：

   - “复制迁移（推荐）”
   - “继续使用旧目录”
6. 选择“复制迁移（推荐）”。
7. 预期：复制完成后会再次弹窗询问是否删除旧目录以释放空间（默认不删除）。

#### 4) 验证“复制迁移确实成功”的检查点

1. 应用启动进入主界面后，进入 `Settings -> Diagnostics -> 刷新`。
2. 找到 `data_dir` 字段：
   - 预期：`data_dir` 指向新目录（`...\edge-video-agent-data`）。
3. 用资源管理器打开该目录，检查是否存在你在旧目录创建的哨兵文件：
   - 预期：新目录中应能找到 `__MIGRATION_SENTINEL__.txt`。
4. 如果你在第二个弹窗选择了“删除旧目录”：
   - 预期：`%USERPROFILE%\.edge-video-agent` 被删除（或至少不存在/为空）。

#### 5) 验证“一键导出 zip”步骤（打包版 Settings 内）

1. 打包版应用打开后：进入 `Settings -> Diagnostics`。
2. 点击：
   - “导出数据（zip）”
3. 选择保存位置（建议保存到 `Downloads`）。
4. 预期：提示导出成功，并返回 zip 文件路径。
5. （可选）检查 zip 内容：
   - zip 里通常会包含一个顶层目录（例如 `edge-video-agent-data`），这是正常的。

#### 6) 验证“一键恢复 zip”步骤（打包版 Settings 内）

> 注意：恢复会覆盖当前数据。实现上会先把当前数据目录重命名为 `*.backup-<timestamp>`，再切换到恢复后的数据。

1. 进入 `Settings -> Diagnostics`。
2. 点击：
   - “恢复数据（zip）”
3. 选择刚才导出的 zip。
4. 会弹出“覆盖确认”对话框，点击继续。
5. 预期：
   - 当前数据目录会被备份为 `...backup-<timestamp>`（同级目录）
   - 恢复完成后应用会尝试重启后端并刷新页面
6. 再次 `Settings -> Diagnostics -> 刷新`：
   - 预期：`data_dir` 不变
   - 数据内容回到备份时的状态
...
- **看不到迁移弹窗**：
  - 你运行的不是打包版（开发模式不会触发）。
  - 新目录不是空的（迁移策略是“仅在新目录为空时”才提示迁移，避免覆盖已有数据）。
- **Settings 里看不到“导出/恢复”按钮**：
  - 你运行的不是打包版（按钮仅打包版显示）。
...
- 2026-02-22：代码质量与工程文档优化：

  - 为后端数据库层（`db.py`, `repo.py`）以及前端核心组件（`VideoDetailPage.tsx`）等文件添加并完善了关键的中文工程架构和设计说明注释。
  - 整理了 `PROJECT_STATUS.md` 中冗余零散的笔记记录，保持项目状态同步文档整洁。
- 2026-02-23：LLM 健壮性防护改进：

  - `main.py`：`POST /videos/{video_id}/summarize` 接口在创建任务前检查 LLM provider 配置；若当前 provider 为 `fake`/`none`，在响应 202 JSON 中附加 `warning` 字段，提示用户去 Settings 中正确配置 LLM，避免静默生成无效摘要。
  - `runtime.py`：将 LLM 并发锁等待超时默认值从 3 秒提升至 **30 秒**，ASR 和重型任务（Heavy）从 3 秒提升至 **10 秒**，防止高负载下因信号量等待超时导致任务意外失败。以上超时值均可通过环境变量（`LLM_CONCURRENCY_TIMEOUT_SECONDS` 等）覆盖。
